/**
 * Individual KYC step: biodata form → init KYC → poll status.
 */
...
import { submitIndividualBiodata, initKyc, getKycStatus } from "@/services/kyc.service";
...
export default function IndividualStep() {
  ...
  const handleSubmit = async () => {
    if (!form.firstName.trim() || !form.lastName.trim() || !form.dateOfBirth || !form.country.trim()) {
      setError("Please fill required fields: first name, last name, date of birth, country.");
      return;
    }
    setLoading(true);
    setError(null);
    try {
      await submitIndividualBiodata({
